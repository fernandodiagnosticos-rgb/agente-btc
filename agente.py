...
def analisar_com_claude(df,score):
    ...
    print(resposta)
    enviar_telegram(f"SINAL BTC Score:{score}\n\n{resposta[:3000]}")
    return resposta
def rodar_agente(ciclos=3,mins=1):
    print("AGENTE INICIADO")
    enviar_telegram("AGENTE BTC INICIADO")
    for i in range(ciclos):
        ...
        if abs(score)>=60:
            ...
            enviar_telegram(f"Score:{score} - Aguardando sinal...")
        if i<ciclos-1:
            time.sleep(mins*60)
    print("AGENTE FINALIZADO")
    enviar_telegram("AGENTE BTC FINALIZADO")
