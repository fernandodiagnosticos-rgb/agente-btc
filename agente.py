...
def calcular_score(df):
    score=0
    p=float(df["fechamento"].iloc[-1])
    ab=sum([p<float(df["EMA9"].iloc[-1]),p<float(df["EMA21"].iloc[-1]),p<float(df["EMA50"].iloc[-1]),p<float(df["EMA200"].iloc[-1])])
    score+=25-(ab*12.5)
    h=float(df["MACDh_12_26_9"].iloc[-1])
    ha=float(df["MACDh_12_26_9"].iloc[-2])
    ...
    k=float(df["STOCHRSIk_14_14_3_3"].iloc[-1])
    d=float(df["STOCHRSId_14_14_3_3"].iloc[-1])
    if k<20:score-=10
    elif k>80:score+=10
    elif k>d:score+=5
    else:score-=5
